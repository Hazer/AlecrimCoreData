...
public class DataContext: ChildDataContext {
    ...
    public init(options: DataContextOptions) {
        do {
            let rootSavingDataContext = try RootSavingDataContext(dataContextOptions: options)
...
public class RootSavingDataContext: ManagedObjectContext {
    ...
    private func assignPersistentStoreCoordinator() throws {
        ...
        case .SQLite:
            ...
            do {
                try persistentStoreCoordinator.addPersistentStore(ofType: NSSQLiteStoreType, configurationName: self.dataContextOptions.configuration, at: persistentStoreURL as URL, options: self.dataContextOptions.options)
            }
                ...
                if error.domain == NSCocoaErrorDomain {
                    ...
                    if migrationErrorCodes.contains(error.code) {
                        handled = self.handleMigrationError(error: error)
                    }
                }
        ...
        case .InMemory:
            try persistentStoreCoordinator.addPersistentStore(ofType: NSInMemoryStoreType, configurationName: self.dataContextOptions.configuration, at: nil, options: self.dataContextOptions.options)
        }
...
public class ChildDataContext: ManagedObjectContext {
    ...
    public override func addObservers() {
        ...
        self.addObserverForName(name: NSNotification.Name.NSManagedObjectContextDidSave.rawValue, object: self.rootSavingDataContext) { [unowned self] notification in
            guard
            ...
            self.perform {
                ...
                //
                self.mergeChanges(fromContextDidSave: notification as Notification)
            }
        }
    }
...
public class ManagedObjectContext: NSManagedObjectContext {
    ...
    public func addObservers() {
        ...
        self.addObserverForName(name: NSNotification.Name.NSManagedObjectContextWillSave.rawValue, object: self) { notification in
    ...
    public final func addObserverForName(name: String, object: AnyObject, closure: @escaping (Notification) -> Void) {
        let observer = NotificationCenter.default.addObserver(forName: Notification.Name(rawValue: name), object: object, queue: nil, using: closure)
